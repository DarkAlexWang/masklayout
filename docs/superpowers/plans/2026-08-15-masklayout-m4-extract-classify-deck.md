...
**Known risks.** Two, both stated rather than hidden. `local_density` is **pattern density: covered area over window area**, computed by unioning the polygon/window intersections so overlaps cannot push it past one. An earlier draft of this plan proposed a count-of-neighbours-over-total ratio and called it "honest but weak"; implementing it showed it was worse than weak, it was degenerate — one bar gives 1/1 and three bars give 3/3, both 1.0, measuring nothing. The test now pins exact values (0.1 for one 100 nm bar in a 1 µm window, 0.3 for three) rather than asserting a relative ordering, which is precisely what let the degenerate version look plausible. And the line-end heuristic is a ratio test with an antiparallel check — it will misfire on a short edge between two long ones that is not a line end, such as a chamfer. `line_end_ratio` is a parameter so that misfire is tunable rather than baked in.
